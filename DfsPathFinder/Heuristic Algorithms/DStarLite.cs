...
namespace PathFinder.Heuristic_Algorithms
{
    class DStarLite
    {
	    ...
	    public bool IsInClosedList(Cell maze_cell)
        {
        ...
        public void printknownmaze(Maze maze)
        {
            ...
            for (tmpcell = goal; tmpcell != start; tmpcell = tmpcell.SearchTree)
                graph[tmpcell.GetMazeCell().X, tmpcell.GetMazeCell().Y].GetMazeCell().SetPathFlag();
            maze.cells[start.GetMazeCell().Y, start.GetMazeCell().X].BackColor = System.Drawing.Color.Gold;
            maze.cells[goal.GetMazeCell().Y, goal.GetMazeCell().X].BackColor = System.Drawing.Color.Blue;
        }
        #endregion
    }
}
