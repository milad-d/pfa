...
namespace PathFinder.Heuristic_Algorithms
{
    class GAAStarLazy
    {
        ...
        //private HashSet<MazeCell> unblocked_cells;
        private Hashtable unblocked_cells;
        ...
        public int GetMazeCellG(Cell maze_cell)
        {
            return graph[maze_cell.Y, maze_cell.X].g;
        }

        public int GetMazeCellH(Cell maze_cell)
        {
        ...
        public bool HasBeenVisitedinLastExecution(Cell maze_cell)
        {
        ...
        public void InformNewGoal(Cell new_goal)
        {
            this.new_goal = graph[new_goal.Y, new_goal.X];
        }

        public void InformNewStart(Cell new_start)
        {
            start = graph[new_start.Y, new_start.X];
        }

        public void InformUnblockedCells(Hashtable unblocked_cells)//(HashSet<Cell> unblocked_cells)
        {
            this.unblocked_cells = unblocked_cells;
        }

        public bool IsInOpenList(Cell maze_cell)
        {
            ...
            while (open_list.Size() > 0 && goal.g > ((GAAStarNode)open_list.Peek()).f)
            {
                ...
                for (int i = 0; i < neighborhood; i++)
                {
                    ...
                    if (0 <= x && x < w && 0 <= y && y < h)
                    {
                        ...
                        if (child.GetMazeCell().IsBlocked()) continue;
