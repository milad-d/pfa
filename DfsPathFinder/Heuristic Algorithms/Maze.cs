...
namespace PathFinder.Heuristic_Algorithms
{
    public class Maze
    {
...

            {
                ...
                for (int y = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
                    {
                        ...
                        drawer.DrawMazeCell(bg2, options, cells[y,x]);
        ...
        public void SetGoal(Cell maze_cell)
        {
	        goal = maze_cell;
	        if(maze_cell.IsBlocked()) maze_cell.SetCost(1);
        }

        public void SetStart(Cell maze_cell)
        {
	        start = maze_cell;
	        if(maze_cell.IsBlocked()) maze_cell.SetCost(1);
        }
